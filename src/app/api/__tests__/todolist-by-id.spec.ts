 ...
 */
import { GET, DELETE } from '../todolists/[id]/route';
...
describe('/api/todolists/[id]', () => {
  ...
  describe('DELETE', () => {
    ...
    // Add mocks for DELETE operations
    const mockTodoListFindByIdAndDelete =
      TodoList.findByIdAndDelete as jest.MockedFunction<
        typeof TodoList.findByIdAndDelete
      >;
    const mockTodoItemDeleteMany = TodoItem.deleteMany as jest.MockedFunction<
      typeof TodoItem.deleteMany
    >;
    ...
    it('should delete a todo list and its items when valid ID and authentication', async () => {
      mockConnectDB.mockResolvedValue(undefined);
      mockGetServerSession.mockResolvedValue(mockSession);
      mockTodoListFindById.mockResolvedValue(mockTodoList);
      mockTodoItemDeleteMany.mockResolvedValue({
        acknowledged: true,
        deletedCount: 3,
      });
      mockTodoListFindByIdAndDelete.mockResolvedValue(mockTodoList);
      ...
      expect(responseData).toEqual({
        data: {
          deletedTodoList: mockTodoList,
          deletedItemsCount: 3,
        },
        message: 'TodoList deleted successfully with 3 associated items',
      });

      expect(mockTodoListFindById).toHaveBeenCalledWith(
        '507f1f77bcf86cd799439011'
      );
      expect(mockTodoItemDeleteMany).toHaveBeenCalledWith({
        todoListId: '507f1f77bcf86cd799439011',
      });
      expect(mockTodoListFindByIdAndDelete).toHaveBeenCalledWith(
        '507f1f77bcf86cd799439011'
      );
    });

    it('should delete todo list with no items', async () => {
      mockConnectDB.mockResolvedValue(undefined);
      mockGetServerSession.mockResolvedValue(mockSession);
      mockTodoListFindById.mockResolvedValue(mockTodoList);
      mockTodoItemDeleteMany.mockResolvedValue({
      ...
      expect(responseData.message).toBe(
        'TodoList deleted successfully with 0 associated items'
      );
      expect(responseData.data.deletedItemsCount).toBe(0);
    });
    ...
    it('should return 401 when user is not authenticated', async () => {
      ...
      expect(responseData).toEqual({
        error: 'Authentication required',
        message: 'You must be logged in to delete a todo list',
      });

      expect(mockTodoListFindById).not.toHaveBeenCalled();
      expect(mockTodoItemDeleteMany).not.toHaveBeenCalled();
    });
    ...
    it('should return 403 when user tries to delete todo list owned by another user', async () => {
      const todoListOwnedByOtherUser = {
        ...mockTodoList,
        userId: 'other-user-456',
      };
    ...
    it('should return 500 when database error occurs', async () => {
      ...
      expect(responseData).toEqual({
        error: 'Internal server error',
        message: 'An unexpected error occurred while deleting the todo list',
      });
    });
  });
});
