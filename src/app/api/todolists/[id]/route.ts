...
export async function DELETE(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    // Delete all todo items associated with this todo list (cascade deletion)
    const deletedItems = await TodoItem.deleteMany({ todoListId: id });
    ...
    return NextResponse.json(
      {
        ...
        message: `TodoList deleted successfully with ${deletedItems.deletedCount} associated items`,
      },
      { status: 200 }
    );
  } catch (error) {
